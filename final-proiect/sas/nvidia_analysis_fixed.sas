/* ========================================================================= */
/* NVIDIA Sales Data Analysis - SAS Programming (FIXED VERSION)             */
...
title "NVIDIA Sales Data Analysis - SAS Implementation (Fixed)";
...
data nvidia_sales_renamed;
    set nvidia_sales;
    Product_Category = "Product Category"n;
    Product_Name = "Product Name"n;
    Sales_Revenue__USD_ = "Sales Revenue (USD)"n;
    Units_Sold = "Units Sold"n;
    Customer_Segment = "Customer Segment"n;
    Customer_Type = "Customer Type"n;
    Market_Share____ = 'Market Share (%)'n;
    drop "Product Category"n "Product Name"n "Sales Revenue (USD)"n "Units Sold"n
         "Customer Segment"n "Customer Type"n 'Market Share (%)'n;
run;

/* Display basic information about the dataset */
proc contents data=nvidia_sales_renamed;
    title2 "Dataset Structure and Variables (Renamed)";
run;

proc print data=nvidia_sales_renamed (obs=5);
    title2 "Sample Data to Check Variable Names (Renamed)";
run;
...
data nvidia_processed;
    ...
    /* Note: Variable names may have underscores due to SAS conversion */
    ...
    if not missing(Sales_Revenue__USD_) and Sales_Revenue__USD_ > 1000000 then customer_value_score + 3;
    if not missing(Units_Sold) and Units_Sold > 3000 then customer_value_score + 2;
    if not missing(Market_Share____) and Market_Share____ > 50 then customer_value_score + 2;
...
data nvidia_calculated;
    ...
    /* Date functions */
    if not missing(date_formatted) then do;
    ...
    if not missing(Sales_Revenue__USD_) and not missing(Units_Sold) and Units_Sold > 0 then do;
        revenue_per_unit = round(Sales_Revenue__USD_ / Units_Sold, 0.01);
    end;
    if not missing(Sales_Revenue__USD_) and Sales_Revenue__USD_ > 0 then do;
        log_revenue = log(Sales_Revenue__USD_);
    end;
    if not missing(Units_Sold) and Units_Sold > 0 then do;
        sqrt_units = sqrt(Units_Sold);
    end;

    /* String functions */
    if not missing(Product_Name) then do;
        product_length = length(Product_Name);
    end;
    if not missing(Region) then do;
        region_upper = upcase(Region);
    end;
    if not missing(Customer_Segment) and not missing(Customer_Type) then do;
        customer_concat = catx(" - ", Customer_Segment, Customer_Type);
    end;

    /* Statistical functions - rank within the dataset */
    if not missing(Sales_Revenue__USD_) then do;
...
proc sql;
    ...
    where not missing(Sales_Revenue__USD_)
    group by Region;
quit;
...
/* First, calculate means and standard deviations for standardization */
...
data nvidia_arrays;
    set nvidia_merged;
    if _n_ = 1 then set stats_summary;

    /* Array for standardizing numeric variables */
    array nums{3} Sales_Revenue__USD_ Units_Sold Market_Share____;
    array means{3} mean_revenue mean_units mean_market_share;
    array stds{3} std_revenue std_units std_market_share;
    array std_nums{3} std_revenue_calc std_units_calc std_market_share_calc;

    /* Standardize variables using arrays */
    do i = 1 to 3;
        if not missing(nums{i}) and stds{i} > 0 then do;
            std_nums{i} = (nums{i} - means{i}) / stds{i};
        end;
    end;

    /* Array for creating performance indicators */
    array indicators{3} $15 revenue_indicator units_indicator market_indicator;

    do j = 1 to 3;
        if not missing(std_nums{j}) then do;
    ...
    drop i j mean_revenue mean_units mean_market_share std_revenue std_units std_market_share;
run;
...
/* Add observation ID for cluster merging */
data nvidia_arrays_withid;
    set nvidia_arrays;
    obs_id = _N_;
run;
...
/* Cluster analysis */
ods exclude History;
proc cluster data=nvidia_arrays_withid method=ward outtree=tree;
    title2 "Cluster Analysis of Customer Segments";
    var std_revenue_calc std_units_calc std_market_share_calc;
    id obs_id;
run;
ods exclude none;

/* Create clusters and merge back to original data */
proc tree data=tree out=clusters(keep=obs_id cluster) nclusters=4;
run;

proc sort data=clusters; by obs_id; run;
proc sort data=nvidia_arrays_withid; by obs_id; run;

data clusters_full;
    merge nvidia_arrays_withid clusters;
    by obs_id;
run;

/* Analyze clusters */
proc means data=clusters_full;
...
/* End of program */
title;
footnote "Analysis completed: NVIDIA Sales Data - SAS Implementation (Fixed)";
footnote2 "Economic insights provided for strategic business decision making";
footnote3 "All technical issues resolved: variable names, formats, missing data handling"; 